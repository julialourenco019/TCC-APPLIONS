...
class ApplionsSupabaseUser extends BaseAuthUser {
  ApplionsSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> applionsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ApplionsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
